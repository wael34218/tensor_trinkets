...
with tf.name_scope("Output") as scope:
    ...
    # outputs[-1] to take the final output
...
with tf.Session() as sess:
    ...
    # Hack to calculate testing accuracy. Wasnt able to make the network consume testing data
    # all at once, so had to pass it in batches.
    total_batch = int(mnist.test.num_examples/batch_size)
    batch_xs = mnist.test.images.reshape([-1, n_steps, n_inputs])
    test_accuracy = accuracy.eval({x: batch_xs, y: mnist.test.labels})
